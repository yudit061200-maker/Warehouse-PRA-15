...
export const DashboardTab: React.FC<DashboardTabProps> = ({
  ...
  return (
    <div className="space-y-6 w-full pb-10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-5">
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 sm:gap-2.5 shrink-0">
            <button
              ...
              className="px-3.5 py-2.5 sm:py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-xs active:scale-95 cursor-pointer min-h-[40px]"
            >
            ...
            <button
              ...
              className="px-3.5 py-2.5 sm:py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-xs active:scale-95 cursor-pointer min-h-[40px]"
            >
      ...
      {lowStockCount > 0 && (
        <div className="bg-rose-50/80 border border-rose-200/80 rounded-2xl p-3.5 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          ...
          <button
            ...
            className="w-full sm:w-auto px-3.5 py-2 sm:py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold rounded-xl transition-colors shrink-0 self-start sm:self-center shadow-xs cursor-pointer text-center"
          >
            Lihat Restock
          </button>
        </div>
      ...
      {/* 4 Minimalist Stat Cards: 2-column on mobile & tablet, 4-column on desktop */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-3.5 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="text-lg sm:text-2xl font-bold tracking-tight text-slate-900 mt-1.5 sm:mt-2 font-mono truncate">
              {formatNumber(analytics?.totalStockQuantity || 0)}
            </div>
          </div>
          <p className="text-[10px] sm:text-xs text-slate-500 mt-1.5 sm:mt-2 font-medium truncate">
            <span className="font-semibold text-slate-700">{items.length} SKU</span> terdaftar
          </p>
        </div>
        ...
        <div className="bg-white p-3.5 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="text-sm sm:text-xl lg:text-2xl font-bold tracking-tight text-slate-900 mt-1.5 sm:mt-2 font-mono truncate">
              {formatRupiah(analytics?.totalValuation || 0)}
            </div>
          </div>
          <p className="text-[10px] sm:text-xs text-emerald-700 mt-1.5 sm:mt-2 font-medium flex items-center gap-1 truncate">
        ...
        <div className="bg-white p-3.5 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-1">
              ...
              <div className="p-1.5 sm:p-2 rounded-xl bg-slate-100 text-slate-600 shrink-0">
                <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 mt-1.5 sm:mt-2 font-mono">
              <div className="text-emerald-600 min-w-0">
                <span className="text-[9px] sm:text-[10px] text-slate-400 block font-sans font-medium leading-none">
                  Masuk
                </span>
                <span className="text-base sm:text-xl font-bold">+{analytics?.todayInQuantity || 0}</span>
              </div>
              <div className="h-5 sm:h-6 w-px bg-slate-200 shrink-0" />
              <div className="text-rose-600 min-w-0">
                <span className="text-[9px] sm:text-[10px] text-slate-400 block font-sans font-medium leading-none">
                  Keluar
                </span>
                <span className="text-base sm:text-xl font-bold">-{analytics?.todayOutQuantity || 0}</span>
              </div>
            </div>
          </div>
          <p className="text-[10px] sm:text-xs text-slate-500 mt-1.5 sm:mt-2 font-medium truncate">
            {analytics?.todayTransactionsCount || 0} mutasi
          </p>
        </div>

        {/* Stock Alert / Status Card */}
        <div
          className={`p-3.5 sm:p-5 rounded-2xl border shadow-xs flex flex-col justify-between ${
            lowStockCount > 0
        ...
        >
          <div>
            ...
            <div
              className={`text-lg sm:text-2xl font-bold tracking-tight mt-1.5 sm:mt-2 font-mono truncate ${
          ...
          <p className="text-[10px] sm:text-xs text-slate-500 mt-1.5 sm:mt-2 font-medium truncate">
            {outOfStockCount > 0 ? `${outOfStockCount} item kosong` : 'Stok aman'}
          </p>
        </div>
      </div>
