...
export const QRCodeScannerTab: React.FC<QRCodeScannerTabProps> = ({
  ...
  const handleQuickAction = async (type: TransactionType, amount: number) => {
    ...
    try {
      ...
      setSuccessToast(
        `Sukses: ${
          type === 'IN' ? 'Penerimaan' : 'Pengeluaran'
        } ${amount} ${scannedItem.unit} untuk "${scannedItem.name}" berhasil dicatat!`
      );
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg md:text-xl font-bold tracking-tight text-slate-900 flex items-center gap-2.5">
            ...
            <span>Pemindai QR Code & Mutasi Cepat</span>
          </h2>
          <p className="text-xs md:text-sm text-slate-500 mt-1">
            Pindai kode QR fisik pada barang atau rak untuk cek stok seketika dan eksekusi mutasi masuk/keluar
          </p>
        </div>
      </div>
          ...
          {notFoundCode && (
            <div className="p-4 bg-rose-50 border border-rose-200/80 rounded-2xl text-xs text-rose-800 space-y-1.5 animate-in fade-in">
              <div className="flex items-center gap-2 font-bold">
                ...
                <span>QR Code / SKU "{notFoundCode}" Belum Terdaftar</span>
              </div>
              <p className="text-rose-700 leading-relaxed">
                Kode ini belum ada di master inventory. Pastikan barang sudah didaftarkan di Master Barang atau Pedoman Data.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          {currentScannedItem ? (
            <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-slate-50 border border-slate-200 rounded-xl shrink-0 hidden sm:block">
                    <QRCodeRenderer
                      value={currentScannedItem.barcode || currentScannedItem.sku}
                      size={54}
                      includeMargin={false}
                    />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 text-xs text-slate-500 font-mono">
                      ...
                      <span>•</span>
                      <span>Kategori: {currentScannedItem.category}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-slate-100 text-xs">
                <button
                  onClick={() => onPrintQRCode(currentScannedItem)}
            ...
            <div className="bg-white p-10 rounded-2xl border border-slate-200/80 shadow-xs text-center space-y-3">
              ...
              <h3 className="text-sm font-bold text-slate-800">Menunggu Pemindaian QR Code...</h3>
              <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
                Arahkan QR Code pada stiker produk/rak ke arah kamera pemindai di sebelah kiri atau ketik kode secara manual.
              </p>
            </div>
          )}
                ...
                {scanHistory.map((h, i) => (
                  ...
                  >
                    <div>
                      ...
                      <span className="text-[10px] text-slate-400 font-mono">{h.item.sku} • {h.item.location}</span>
                    </div>
                    <span className="text-[10px] text-slate-400 font-mono">{formatDateTime(h.time)}</span>
                  </div>
                ))}
              </div>
            </div>
